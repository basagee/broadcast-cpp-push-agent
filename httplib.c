     ...
     int *pfd;    /* pointer to variable where to set file descriptor value */
{
    ...
    if (connect(s, (struct sockaddr *)&server, sizeof(server)) < 0) {
        ...
        if (proxy) {
            sprintf(header,
                    "%s http://%.128s:%d/%.256s HTTP/1.1\015\012User-Agent: %s\015\012%s\015\012",
                    command,
                    http_server,
                    http_port,
                    url,
            ...
            sprintf(header,
                    "%s /%.256s HTTP/1.1\015\012Host: %s:%d\015\012User-Agent: %s\015\012%s\015\012",
                    command,
                    url,
                    http_server,
                    http_port,
     ...
     char *type;      /* type of the data, if NULL default type is used */
{
    char header[MAXBUF];
    if (type) { 
        sprintf(header, "Content-length: %d\015\012Content-type: %.64s\015\012Accept: %.64s\015\012%s",
                length,
                type  ,
                type  ,
                overwrite ? "Control: overwrite=1\015\012" : ""
...
http_retcode http_post_keepopen(filename, senddata, sendlength, overwrite, typebuf, precvdata, precvlength) 
     char *filename;  /* name of the ressource to create */
     char *senddata;      /* pointer to the data to send   */
     int sendlength;      /* length of the data to send  */
     int overwrite;   /* flag to request to overwrite the ressource if it
                         was already existing */
     char *typebuf;      /* type of the data, if NULL default type is used */
     char **precvdata; /* address of a pointer variable which will be set
                      to point toward allocated memory containing read data.*/
     int  *precvlength;/* address of integer variable which will be set to
                      length of the read data */
{
    char header[MAXBUF];
    char foundChunked[MAXBUF];
    int  fd;
    char *pc;
    int  n,length = -1, isUseChunked;
    ...
    if (typebuf) { 
        ...
        sprintf(header, "Content-length: %d\015\012%s", sendlength,
                overwrite ? "Control: overwrite=1\015\012" : ""
                );
    }
    http_retcode ret = http_query("POST", filename, header, KEEP_OPEN, senddata, sendlength, &fd);
    if (ret == 200) {
        while (1) {
            ...
            sscanf(header, "transfer-encoding: %s", foundChunked);
            if (strlen(foundChunked) > 0 && strncmp(CHUNKED, foundChunked, strlen(CHUNKED)) == 0) {
                isUseChunked = 1;
            }
        }
        ...
        if (isUseChunked) {
            n = http_read_line(fd, header, MAXBUF - 1);
            log(DEBUG, "%s\n", header);
            length = (int)strtol(header, NULL, 16);
            log(DEBUG, "Use transfer-encoding: chunked .. body length = %d", length);
            if (length <= 0) {
                close(fd);
                return ERRNOLG;
            }
        ...
        if (precvlength) *precvlength = length;
        if (!(*precvdata = malloc(length + 1))) {
            close(fd);
            return ERRMEM;
        }
        memset(*precvdata, 0x00, length + 1);
        n = http_read_buffer(fd, *precvdata, length);
        close(fd);
     ...
     char **pfilename; 
{
    ...
    if (strncasecmp("http://", url, 7)) {
        log(ERROR, "invalid url (must start with 'http://')\n");
        return ERRURLH;
    }
    url += 7;
    ...
    *pfilename = strdup ( c ? pc : "") ;
    ...
    log(DEBUG, "host=(%s), port=%d, filename=(%s)\n",
